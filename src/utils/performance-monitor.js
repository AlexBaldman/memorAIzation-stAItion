...
class PerformanceMonitor {
  ...
  updateAIStats() {
    ...
    if (stats.cacheStats) {
      const hitRate =
        stats.cacheStats.total > 0
          ? ((stats.cacheStats.hits / stats.cacheStats.total) * 100).toFixed(1)
          : 0;
    ...
    if (stats.providerStats && Object.keys(stats.providerStats).length > 0) {
      html += `<div class="mt-2 pt-2 border-t border-zinc-600">`;
      Object.entries(stats.providerStats).forEach(
    ...
    return {
      ...
      ai: {
        cacheHitRate: aiStats.cacheStats
          ? (
              (aiStats.cacheStats.hits / aiStats.cacheStats.total) *
              100
            ).toFixed(1)
          : 0,
