...
class MemoryCard {
  ...
  setupAccessibility() {
    // ARIA labels
    this.element.setAttribute('aria-label', `Memory card for ${this.data.name}`);
    this.element.setAttribute('aria-describedby', `card-desc-${this.data.number}`);
  ...
  flip() {
    ...
    // Notify state change
    memoryState.set('pao.selectedCard', this.isFlipped ? this.data.number : null);
  }
  ...
  async handleGenerateImage(e) {
    ...
    try {
      ...
      const result = await aiService.generateImage(prompt, { priority: 'normal' });
  ...
  handleImageCycle(e) {
    ...
    const manifest = memoryState.get('imageManifest');
    if (!manifest || !manifest[this.data.number] || manifest[this.data.number].length <= 1) {
      return;
    }
  ...
  getCurrentImageIndex(images) {
    ...
    return images.findIndex(img => img === currentSrc || img.endsWith(currentSrc.split('/').pop()));
  }
  ...
  getPerformanceMetrics() {
    return {
      ...
      metrics: this.performanceMetrics ? Object.fromEntries(this.performanceMetrics) : {}
    };
  }
